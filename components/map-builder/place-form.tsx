...
export function PlaceForm({
  ...
  const importIsValid = importUrl.trim().length > 0 && !isImporting;
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-border/60">
        <CardHeader>
          <CardTitle className="text-base">Import Google list</CardTitle>
          <CardDescription>
            Paste a shared Google Maps list URL. The server will create a new
            custom map URL from the list metadata and listed places without
            hydrating them.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="google-list-url">Google list URL</Label>
            <div className="relative">
              <Link2 className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                id="google-list-url"
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button type="button" disabled={!importIsValid} onClick={onImport}>
              <Plus className="size-4" />
              {isImporting ? "Importing..." : "Import list"}
            </Button>
            <Button type="button" variant="outline" onClick={onImportReset}>
              <RotateCcw className="size-4" />
              Reset
            </Button>
          </div>
